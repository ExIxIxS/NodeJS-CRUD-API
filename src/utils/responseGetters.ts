  ...
  if (url && url.startsWith('/api/users/')) {
    ...
    if (!isValidUUID(userId)) {
      return getResponseObj(
        400,
        JSON.stringify({ message: INVALID_ID_MESSAGE })
      );
    }
    ...
    return user
      ...
      : getResponseObj(
          404,
          JSON.stringify({ message: USER_NOT_FOUND_MESSAGE })
        );
  }
...
function getPostResponse(requestBody: string) {
  ...
  if (!username || !age || !hobbies?.length || !Array.isArray(hobbies)) {
    return getResponseObj(
      400,
      JSON.stringify({ message: MISSING_FIELDS_MESSAGE })
    );
  } else {
    const newUser: User = {
      id: uuidv4(),
...
function getDeleteResponse(url: string) {
  ...
  return deletedUser
    ? getResponseObj(
        204,
        JSON.stringify({ message: USER_SUCCESSFULLY_DELETE_MESSAGE })
      )
